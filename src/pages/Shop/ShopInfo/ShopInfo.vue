...
<div class="activity">
<div class="activity-item " v-for="(support,index) in shopInfo.supports" :key="index" :class="activityType[support.type]">
...
export default {
    data () {
        return {
      activityType:['activity-green', 'activity-red', 'activity-orange'],   
        }
    },
    mounted () {
      this.$store.dispatch('getShopInfo',()=>{
       this.$nextTick(()=>{
        ...
        const ul = this.$refs.picWrapper
        const count =this.shopInfo.pics.length
       const liWidth = 120
       const space = 6
        ul.style.width = (liWidth+space)*count -space +'px'
      if(!this.wrapper){
           this.wrapper =  new BScroll('.pic-wrapper',{
            click:true,
            scrollX:true //水平滑动
        })
      }else{
